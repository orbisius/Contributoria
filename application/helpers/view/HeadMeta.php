...
/**
 * 
 * Class to improve HTML5 meta tags
 * 
 * @author daniellevitt
 *
 */
class Helper_View_HeadMeta {

    protected $_typeKeys = array(
        'name', 'http-equiv', 'charset', 'property'
    );
    ...
    protected function _isValid($item) {
        ...
        if (!isset($item->content) && (($doctype !== 'HTML5' && $doctype !== 'XHTML5') || (($doctype == 'HTML5' || $doctype == 'XHTML5') && $item->type !== 'charset'))) {
            return false;
        }
    ...
    public function itemToString(stdClass $item) {
        ...
        if ($this->view instanceof Zend_View_Abstract) {
            if ($this->view->doctype()->getDoctype() == 'XHTML5' && $type == 'charset') {
                $tpl = '<meta %s="%s"/>';
            } elseif ($this->view->doctype()->getDoctype() == 'HTML5' && $type == 'charset') {
                $tpl = '<meta %s="%s">';
